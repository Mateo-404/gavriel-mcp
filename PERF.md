...
- **Listing de tools** (name + title + description): una vez por sesión.
  - Antes: ~9.100 chars ≈ **~2.550 tok** (66 descripciones + 65 títulos + 53 `.describe()`).
  - Después: **2.337 tok (full / 65 tools)**; **1.155 tok (readonly / 38 tools)**.
  - Reducción ~8% en descripciones (ya eran tersas; se ahorra relleno sin perder
    hints operativos: "backend lento, filtrar siempre", "+170k tickets",
    estados válidos, `confirm` obligatorio).
...
| F1 | `outputSchema` + structured en las 5 tools más pesadas: list_tickets, get_ticket, list_events, get_account, list_conversations | Habilita que el cliente reduzca el tool result; el texto es un resumen, no un espejo | keep |
| G1 | Test e2e de roles via `listTools` (InMemoryTransport): readonly sin las 27 de escritura, full con todas | Verifica el feature de roles sin red | keep |
